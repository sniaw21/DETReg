...
def make_lol_transforms(image_set):
    ...
    scales = [720, 768, 816, 864, 912, 960, 1007, 1056, 1104, 1152, 1200]
...
def build(image_set, args):
    root = Path(args.lol_path)
    assert root.exists(), f'provided LOL path {root} does not exist'
    mode = 'instances'
    PATHS = {
    ...
    if image_set == 'viz':
        ...
        dataset = VizOnly(img_folder, transforms=make_lol_transforms('val'))
        return dataset
    ...
    dataset = CocoDetection(img_folder, ann_file, transforms=make_lol_transforms(image_set), return_masks=args.masks,
